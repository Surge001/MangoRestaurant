...
namespace IdentityServerHost.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            {
                var blah = await this.signinManager.PasswordSignInAsync(model.Username, model.Password, model.RememberLogin, lockoutOnFailure: false);
                if (blah.Succeeded)
                {
                    var user = await this.userManager.FindByNameAsync(model.Username);
                    await this._events.RaiseAsync(new UserLoginSuccessEvent(model.Username, user.Id, user.UserName, clientId: context?.Client.ClientId));
                    if(context != null)
                    {
                        return Redirect(model.ReturnUrl);
                    }
                    ...
                    {
                        throw new Exception("Bad return URL");
                    }
                }
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (!this.roleManager.RoleExistsAsync(model.RoleName).GetAwaiter().GetResult())
                    {
                    ...
                    var context = await _interaction.GetAuthorizationContextAsync(model.ReturnUrl);
                    var loginresult = await this.signinManager.PasswordSignInAsync(model.Username, model.Password, false, lockoutOnFailure: true);
                    if (loginresult.Succeeded)
                    {
                        var checkuser = await this.userManager.FindByNameAsync(model.Username);
